...
unsigned char * NEAR SRAMalloc(NEAR unsigned char nBytes)
{
    	...
    	#ifdef ENABLE_DEBUG
        	UART2PutROMString( (ROM char * const)"Alloc " );
	        PrintChar(nBytes);
			printf("\r\n");
    	#endif
	    ...
	    {
    	    #ifdef ENABLE_DEBUG
        	    UART2PutROMString( (ROM char * const)" check " );
        	...
        	{
            	#ifdef ENABLE_DEBUG
                	UART2PutROMString( (ROM char * const)" TableEnd\r\n" );
	            #endif
    	        #ifdef MAKE_INTERRUPT_SAFE
        	        INTCONbits.GIEH = saveGIEH;
            	#endif
	        ...
	        if (!(segHeader.bits.alloc))
    	    {
	            ...
	            {
    	            #ifdef ENABLE_DEBUG
        	            UART2PutROMString( (ROM char * const)" merge" );
            	    #endif
	            ...
	            if (nBytes == segLen)
    	        {
        	        ...
        	        #endif
            	    #ifdef ENABLE_DEBUG
                	    UART2PutROMString( (ROM char * const)" Found\r\n" );
	                #endif
...
void SRAMfree(unsigned char * NEAR pSRAM)
{
    ...
    #ifdef ENABLE_DEBUG
        UART2PutROMString( (ROM char * const)"Free\r\n " );
        PrintChar((*(SALLOC *)(pSRAM - 1)).byte - 1);
        UART2PutROMString( (ROM char * const)" at " );
